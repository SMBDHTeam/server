...
public interface PostMediaRepository extends JpaRepository<PostMedia, Long> {
     ...
     *
     * <p><b>축소본 주소도 함께 본다.</b> 축소본은 저장소에 따로 놓인 객체라 목록에 잡히는데,
     * 원본 주소만 맞대 보면 쓰이고 있는 축소본을 고아로 보고 지운다. 그러면 목록의 사진이
     * 전부 깨진다.
     */
    @Query("""
            select m.url from PostMedia m where m.url in :urls
            union
            select m.thumbnailUrl from PostMedia m where m.thumbnailUrl in :urls
            """)
     ...
     *
     * <p>축소본도 지워야 한다. 원본만 지우면 아무도 참조하지 않는 축소본이 저장소에 남고,
     * 게시물이 사라져 정리 배치도 근거를 잃는다.
     */
    @Query("""
            select m.url from PostMedia m where m.post.id in :postIds
            union
            select m.thumbnailUrl from PostMedia m
            where m.post.id in :postIds and m.thumbnailUrl is not null
            """)
