...
public interface PostHashtagRepository extends JpaRepository<PostHashtag, PostHashtagId> {
     ...
     *
     * <p>해시태그는 장소가 아니라 글에 붙는다. 한 글에 장소를 여러 곳 태그하면 그 태그가
     * 어느 곳을 가리키는지 알 수 없으므로, <b>장소를 하나만 태그한 글</b>만 센다.
     ...
     *
     * <p>그래도 사용자가 만드는 데이터라 어긋난 태그가 섞일 수 있다. 화면에서 확정된
     * 사실이 아니라 사용자들이 붙인 태그임을 알 수 있게 보여줘야 한다.
     */
    @Query("""
            ...
            where link.hashtag.name = :name
              ...
              and (select count(other) from PostPlaceTag other
