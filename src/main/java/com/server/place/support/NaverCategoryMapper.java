 ...
 * content type. Without one, the Planner falls back to a flat dwell time and
 * {@link TourApiThemeMapper}'s content type rules never fire, so a place the traveller picked
 * themselves would be planned worse than an ingested one. Deriving a content type at
 * registration time keeps both paths identical.
 */
