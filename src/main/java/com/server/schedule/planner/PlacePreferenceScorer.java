...
public class PlacePreferenceScorer {
    ...
    public int themeScore(Place place, ScheduleCreateRequest request) {
        String answerId = answerId(request, "THEME").orElse("");
        String contentTypeId = place.getContentTypeId();
        String name = name(place);
        return switch (answerId) {
            case "THEME_FOOD" -> Objects.equals(contentTypeId, "39")
                    ? STRONG_PREFERENCE_BONUS : PREFERENCE_MISMATCH_PENALTY;
            case "THEME_HISTORY_CULTURE" -> Objects.equals(contentTypeId, "14")
                    || containsAny(name, "역사", "문화", "박물관", "기념관", "유적")
                    ? STRONG_PREFERENCE_BONUS : PREFERENCE_MISMATCH_PENALTY;
            case "THEME_NATURE" -> Objects.equals(contentTypeId, "12")
                    && containsAny(name, "해수욕장", "바다", "공원", "산책로", "섬", "숲")
                    ? PREFERENCE_BONUS : 0;
            case "THEME_NIGHT_VIEW" -> containsAny(name, "야경", "전망", "타워", "광안", "해변")
                    ? PREFERENCE_BONUS : 0;
            case "THEME_EVENT" -> Objects.equals(contentTypeId, "15")
                    ? STRONG_PREFERENCE_BONUS : PREFERENCE_MISMATCH_PENALTY;
            case "THEME_LOCAL" -> containsAny(name, "로컬", "시장", "거리", "마을", "골목")
                    ? PREFERENCE_BONUS : 0;
            default -> 0;
        };
    }
    ...
    private int walkPenalty(int nearestEndpointDistance, ScheduleCreateRequest request) {
        ...
        if (hasAnswerContaining(request, "PACE", "FAST") || hasAnswerContaining(request, "PACE", "ACTIVE")) {
            comfortMeters = ACTIVE_WALK_COMFORT_METERS;
        }
