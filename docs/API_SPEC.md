...
`mustVisitPlaceIds`는 사용자가 장소 검색에서 선택한 내부 `places.id` 목록이다. 생략할 수 있으며 중복 없이 여행 일수당 최대 3개까지 전달할 수 있다. 전달된 장소는 일정 방문지와 대중교통 경로에 반드시 포함하고 남은 슬롯은 자동 추천으로 채운다.
...
`evaluation`은 생성 시점의 요청과 Planner 실행을 평가한 결과다. Hard Gate 위반 일정은 저장하거나 반환하지 않으며 `400 INVALID_SCHEDULE_CONDITION`으로 실패한다. `qualityScore`는 Hard Gate를 통과한 일정의 100점 품질표이고, `operations`는 생성시간, 경로 탐색·캐시, 외부 Provider 호출과 최종 일정 경로의 운영 지표다. Planner는 후보 순서 비교에서 경량 경로를 사용하고, 최종 선택 경로는 경량 검색 결과를 재사용해 상세 선형·실시간·도보 정보만 보강한다. `providerCallCount`는 두 단계의 내부 `TransitRouteProvider` 호출 수 합계이고, `routeResolutionCount`와 `routeCacheHitCount`는 후보 탐색과 최종 상세화 단계의 합계다. `externalHttpCallCount`는 ODsay 경로검색·`loadLane`·TMAP 도보 요청을 합친 실제 HTTP 시도 수다. `geometryFallbackLineCount`는 상세 선형을 얻지 못해 단순 좌표를 사용한 지도 경로선 수다.
