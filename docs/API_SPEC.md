...
| 구분 | 대상 |
| --- | --- |
| 로그인 없이 | 피드, 인기 피드, 게시물 상세, 댓글 목록, 프로필, 사용자 게시물, 검색, 카테고리 |
| 로그인 필요 | 작성·수정·삭제·복구, 좋아요, 저장, 댓글 쓰기, 팔로우, 차단, 신고, 업로드 |
| 로그인 필요 | 내 알림, 내 저장 목록, 내 차단 목록, 내가 지운 게시물 |

마지막 줄은 조회지만 **남의 것이 아니라 내 것**이라 로그인이 필요하다.
...
**지운 게시물의 파일은 아직 지우지 않는다.** 게시물을 삭제해도 저장소의 파일은 남는다.
30일 뒤 게시물이 완전히 사라질 때 함께 지우는 처리가 아직 없다. 아래 `C-18` 에 적었다.
...
| 항목 | 상태 |
| --- | --- |
| 지운 게시물의 파일 삭제 | 게시물만 지우고 저장소의 파일은 남는다 |
