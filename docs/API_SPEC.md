...
작성 버튼을 눌렀을 때 기다리지 않는다. 자세한 계약은 `C-17`을 본다.

서버는 `url` 이 우리 버킷 주소인지 확인하지 않는다. 지금은 인증이 없어 확인해도 막을 수 있는
것이 없고, 인증이 붙으면 그때 넣는다.

**카테고리는 본문과 무관하다.** 사용자가 목록에서 고른 값을 `categories` 로 보낸다. 본문에
`#맛집` 을 써도 카테고리가 되지 않는다. 두 경로를 함께 두면 같은 글이 어떤 화면에서는 잡히고
어떤 화면에서는 빠진다.

쓸 수 있는 카테고리는 여덟 개이며 화면의 탭과 같다. 목록은 `GET /api/v1/categories` 로 받는다.

```
맛집  카페  힐링  액티비티  쇼핑  야경  역사  자연
```

화면의 "전체" 탭은 필터 없이 조회하는 것이라 카테고리가 아니다.

**등록된 것만 붙는다.** 사용자가 새로 만들 수 없다. 목록에 없는 이름을 보내도 요청은 성공하고
그 이름만 무시한다. 오타 하나로 글 작성이 막히면 안 되고, 카테고리는 분류를 돕는 값이지 글의
필수 조건이 아니다. 무시된 이름은 응답 `categories` 에도 없다.

```
요청  "categories": ["맛집", "없는이름"]
응답  "categories": ["맛집"]
```

자유 입력을 허용하면 같은 뜻이 표기만 달리해 흩어지고, 카테고리로 거르는 화면이 제 구실을
못 한다. 클라이언트는 직접 입력하게 두지 말고 목록에서 고르게 한다.
...
**교체로 빠진 사진의 실제 파일은 저장소에서도 지운다.** 수정이 커밋된 뒤에 지우므로 응답이
늦어지지 않고, 지우지 못해도 수정은 그대로 성공한다. 같은 `url` 을 다시 보낸 사진은 남긴다.
`C-17`에 적었다.
...
`category`를 함께 보내면 그 태그가 달린 게시물만 남는다. **화면의 카테고리 탭은 최신 피드와
인기 피드에 같은 값을 넘긴다.** 탭을 눌렀는데 한쪽만 걸러지면 목록이 서로 어긋나 보인다.

```
GET /api/v1/posts?category=맛집              최신순
GET /api/v1/posts/popular?category=맛집      인기순
```
...
차단 응답의 `following`은 처리 후 상태다. 차단 직후에는 방금 끊었으므로 항상 `false`이고,
차단을 풀 때는 지금 상태를 다시 읽어 준다. 화면이 프로필을 다시 부르지 않고 팔로우 버튼까지
함께 되돌릴 수 있게 하기 위함이다.
...
`following`은 요청자가 이 사용자를 팔로우 중인지, `blocked`는 차단한 상태인지, `me`는 본인
프로필인지다. 셋 다 요청자 기준이며, 이 조회는 로그인이 필요하다. `me`가 `true`면 북마크
탭을 노출한다.

**`blocked`가 없으면 차단을 푼 뒤 다시 차단할 방법이 없다.** 버튼을 누른 직후에는 차단
응답으로 알 수 있지만, 프로필을 다시 열면 그 상태를 알 도리가 없어 화면이 차단 버튼을
그리지 못한다.
...
`GET /api/v1/categories/search?keyword=광&size=10`

**목록** — `GET /api/v1/categories`

```json
{ "items": [
    { "name": "맛집", "postCount": 12 },
    { "name": "카페", "postCount": 8 }
] }
```

쓸 수 있는 태그 전체를 **등록 순서 그대로** 반환한다. 사용자가 새 태그를 만들 수 없으므로 이
목록이 곧 선택지이며, 화면의 카테고리 탭에 그대로 쓴다.

사용 수로 정렬하지 않는 이유는, 글이 쌓일 때마다 탭 순서가 바뀌면 사용자가 늘 같은 자리에서
같은 탭을 누를 수 없기 때문이다.

```
탭 목록      GET /api/v1/categories
탭 누름      GET /api/v1/posts?category=맛집
"전체" 탭    GET /api/v1/posts
```

**자동완성** — `GET /api/v1/categories/search?keyword=맛&size=10`
...
```json
{ "items": [ { "name": "맛집", "postCount": 1203 } ] }
```

**쓸 수 있는 태그는 서버가 등록해 둔 것이 전부다.** 사용자가 새로 만들 수 없으므로 이 조회
결과가 곧 선택지다. 지금은 여덟 개뿐이라 클라이언트가 목록을 직접 들고 있어도 된다.
...
`GET /api/v1/categories/맛집/places?size=20`
...
이 태그가 달린 게시물들이 태그한 장소를 **언급한 사람이 많은 순**으로 준다. 태그를 눌렀을 때
글 목록 대신 지도를 보여주는 데 쓴다. 관광 데이터에는 "현지인이 가는 곳" 같은 정보가 없으므로,
사용자가 붙인 태그로 그것을 만들어 내는 것이 목적이다.
...
태그를 달려면 그 태그 아래 글이 수백 개는 쌓여야 하는데, 지금은 게시물이 거의 없어 그
기준으로는 아무 장소도 뜨지 않는다. 글이 쌓이면 `COMMUNITY_HASHTAG_PLACE_MIN_AUTHORS` 를
올린다. 코드가 아니라 설정이라 배포 없이 바꿀 수 있다.

`postCount`는 글 수, `authorCount`는 사람 수다. 정렬 기준은 `authorCount`다.

**확정된 분류가 아니다.** 사용자가 만드는 데이터라 어긋난 태그가 섞일 수 있다. 화면에서
"사용자들이 태그한 장소"임을 알 수 있게 보여준다. 잘못된 것은 신고로 처리한다.

### C-13-2. 인기 장소

`GET /api/v1/places/popular?size=10`
...
최근 커뮤니티에서 많이 언급된 장소를 **언급한 사람이 많은 순**으로 준다. 홈 화면의 인기
여행지와 일정 만들 때 장소를 고르는 화면에 쓴다. 로그인 없이 볼 수 있다.

관광 데이터에는 "요즘 사람들이 가는 곳" 같은 정보가 없다. 적재해 둔 값은 이름·주소·분류뿐이라
무엇이 인기인지 가릴 근거가 없다. 사용자가 붙인 장소 태그가 유일한 단서다.

**거르는 규칙은 `C-13` 과 같다.**

| 조건 | 이유 |
| --- | --- |
| 최근 30일 글만 센다 | "지금" 인기 있는 곳을 보여주기 위함이다. 기간이 없으면 작년에 반짝 유행한 곳이 계속 위에 남는다 |
| 장소를 하나만 태그한 게시물만 센다 | 여러 곳을 태그한 글은 어디를 가리키는지 알 수 없다 |
| 서로 다른 사람 2명 이상 | 한 사람이 여러 번 태그해도 순위가 오르지 않고, 혼자 붙인 것은 목록에 뜨지 않는다 |
| 가려 둔 장소는 뺀다 | 눌러서 상세로 들어갔을 때 없는 장소가 되고, 일정에 넣어도 후보로 쓸 수 없다 |

언급한 사람 수와 글 수가 모두 같으면 **최근에 태그된 순**이다. 무작위로 섞으면 새로고침할
때마다 순서가 바뀌어 사용자가 방금 본 장소를 다시 찾지 못한다. 글이 적을 때는 대부분 동점이라
그 흔들림이 더 크게 보인다.

기간과 최소 인원은 설정이라 배포 없이 바꾼다.

```
COMMUNITY_POPULAR_PLACE_DAYS=30
COMMUNITY_POPULAR_PLACE_MIN_AUTHORS=2
```
...
가 보고 싶은 장소를 담아 둔다. 담아 둔 장소는 일정을 만들 때 필수 방문지로 넘긴다.

| 동작 | 요청 |
| --- | --- |
| 담기 | `POST /api/v1/places/{placeId}/wishlists` |
| 빼기 | `DELETE /api/v1/places/{placeId}/wishlists` |
| 내 목록 | `GET /api/v1/users/me/wishlists?page=0&size=20` |

담기·빼기 응답은 처리 후 상태다. 화면이 하트를 바로 바꿀 수 있게 담는다.
...
**최근에 담은 순**이다. 셋 다 로그인이 필요하다. 장소 검색·상세는 인증 없이 열려 있지만
위시리스트는 내 것이라 따로 막는다.

**이미 담은 장소를 다시 담아도 중복 생성되지 않고, 담지 않은 장소를 빼도 오류가 아니다.**
화면의 하트를 두 번 눌러도 같은 결과여야 한다.

**담은 뒤 관리자가 가린 장소는 목록에서 빠진다.** 계속 보여주면 눌러서 상세로 들어갔을 때
없는 장소가 되고, 일정에 넣어도 후보로 쓸 수 없다. 담는 시점에는 막지 않는다. 담긴 뒤에
가려질 수도 있어 어차피 조회에서 걸러야 하고, 두 곳에서 막으면 같은 규칙이 흩어진다.

**일정에 넣을 때는 `placeId` 를 그대로 쓴다.**

```json
POST /api/v1/schedule-previews
{ "mustVisitPlaceIds": [42, 101] }
```

`mustVisitPlaceIds` 는 내부 `places.id` 를 받으므로 위시리스트가 주는 값이 그대로 들어간다.
여행 일수당 최대 5개라는 제약은 그대로다.

없는 장소를 담으려 하면 `404 PLACE_NOT_FOUND` 다.
...
**차단 관계인 사람의 알림도 어느 쪽이 차단했든 빠진다.** 차단한 사람의 이름이 알림함에 계속
뜨면 차단이 무의미해진다. 이미 쌓여 있던 알림도 차단하는 즉시 목록과 `unreadCount`에서
사라지고, 차단을 풀면 다시 나타난다. 행동한 사람이 없는 시스템 알림은 차단과 무관하다.
...
| 항목 | 값 |
| --- | --- |
| 파트 이름 | `files`. 여러 건은 같은 이름을 반복한다 |
| 건수 | 한 번에 1~10건 |
| 크기 | 한 건당 10MB |
| 형식 | `jpg` `jpeg` `png` `webp` `gif` `mp4` `mov` |
...
**확장자와 파일 내용을 함께 본다.** 확장자만 `.jpg` 로 바꾼 실행 파일은 `415` 로 막힌다.
파일 앞머리의 고정 바이트로 판별하며, 응답의 `mediaType` 도 확장자가 아니라 이 결과를 따른다.
...
24시간이 안 된 파일은 건드리지 않는다.** 아직 글을 쓰는 중일 수 있어서다.

이 정리는 `COMMUNITY_MEDIA_ORPHAN_CLEANUP_ENABLED` 로 켠다. 저장소 목록 조회 권한
(`s3:ListBucket`)이 필요해 기본값은 꺼져 있다.
...
`COMMUNITY_MEDIA_S3_ENABLED` 가 꺼진 환경에서는 항상 `503` 이다. 키가 없는 환경에서도
서버가 뜨게 하기 위한 것으로, 다른 위임 계층과 같은 방식이다.
