...
현재 개발 DB는 H2다. 실제 DBMS와 migration 도구가 확정되기 전까지 `json`, `datetime`, UUID의 물리 타입은 확정하지 않으며 아래 논리 모델을 기준으로 Entity를 설계한다.
...
| `token_hash` | varchar | UK, O | 공유 토큰의 단방향 해시 |
...
두루누비 길 정보를 저장한다. 걷기길과 자전거길의 큰 묶음이다.

| 컬럼 | 자료형 | 키·필수 | 의미 |
| --- | --- | --- | --- |
| `id` | bigint | PK, O | 내부 두루누비 길 ID |
| `route_id` | varchar | UK, O | 두루누비 길 고유번호 |
| `theme_name` | varchar | O | 길 이름. 예: `남파랑길` |
| `line_message` | varchar | X | 길 한 줄 설명 |
| `description` | text | X | 길 상세 설명 |
| `brd_div` | varchar | O | 걷기/자전거 구분. `DNWW`, `DNBW` |
| `raw_json` | json | X | 두루누비 `routeList` 원본 응답 |
| `created_at` | datetime | O | 최초 적재시각 |
| `updated_at` | datetime | O | 마지막 갱신시각 |

## 15. durunubi_courses

두루누비 코스 정보를 저장한다. 사용자가 선택하면 일정 생성의 참고 조건으로 사용한다.

| 컬럼 | 자료형 | 키·필수 | 의미 |
| --- | --- | --- | --- |
| `id` | bigint | PK, O | 내부 두루누비 코스 ID |
| `course_id` | varchar | UK, O | 두루누비 코스 고유번호 |
| `route_id` | varchar | FK, X | 소속 두루누비 길 고유번호 |
| `course_name` | varchar | O | 코스명 |
| `level` | integer | X | 난이도. `1`: 하, `2`: 중, `3`: 상 |
| `distance_km` | decimal | X | 코스 거리(km) |
| `required_time_minutes` | integer | X | 예상 소요시간(분) |
| `cycle` | varchar | X | 순환형·비순환형 등 코스 형태 |
| `summary` | text | X | 코스 설명 요약 |
| `contents` | text | X | 코스 상세 설명 |
| `tour_info` | text | X | 주변 관광정보 |
| `traveler_info` | text | X | 교통 및 여행자 안내 |
| `sigun` | varchar | X | 시군구 정보 |
| `gpx_path` | text | X | GPX 파일 경로 |
| `brd_div` | varchar | O | 걷기/자전거 구분. `DNWW`, `DNBW` |
| `raw_json` | json | X | 두루누비 `courseList` 원본 응답 |
| `created_at` | datetime | O | 최초 적재시각 |
| `updated_at` | datetime | O | 마지막 갱신시각 |
...
| `durunubi_routes` 1 : N `durunubi_courses` | 길 하나는 여러 코스를 가진다 |
...
두루누비 코스 선택값은 일정 생성 조건 스냅샷인 `schedules.condition_json`에 저장한다. 1차 스프린트에서는 코스를 별도 방문 계획으로 저장하지 않는다.
