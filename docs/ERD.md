...
| `thumbnail_url` | text | X | 목록용 축소본 URL. 업로드 때 원본과 함께 만든다 |
| `sort_order` | integer | O | 게시물 안의 표시 순서 |

파일 업로드는 `POST /api/v1/media` 가 받아 S3 에 올리고 URL 만 이 표에 남는다.

**`thumbnail_url` 은 비어 있을 수 있다.** 영상, ImageIO 가 읽지 못하는 `webp`, 이미 작은
사진, 그리고 이 열이 생긴 `V22` 이전에 저장된 사진이다. 화면은 이때 `url` 을 쓴다.

축소본은 저장소에 따로 놓인 객체다. 게시물을 완전히 지울 때와 글에 붙지 않은 파일을 정리할
때 두 주소를 함께 다룬다. 한쪽만 보면 쓰이는 사본을 지우거나 버려진 사본이 남는다.
...
카테고리 성격의 태그다. 사용자가 게시물을 쓸 때 **여기 등록된 이름 중에서 고른다.**
새 태그를 만들 수 없고, 본문에서 뽑지도 않는다.

| 컬럼 | 자료형 | 키·필수 | 의미 |
| --- | --- | --- | --- |
| `id` | bigint | PK, O | 태그 ID |
| `name` | varchar | UK, O | 카테고리 이름. 소문자로 정규화해 저장한다 |
| `post_count` | integer | O | 이 태그가 달린 게시물 수. 자동완성 정렬 기준 |

게시물 하나에 최대 10개까지 붙는다.

행은 `HashtagSeedInitializer`가 기동할 때 여덟 개를 넣는다. 등록되지 않은 이름을 보내도
무시할 뿐 새로 만들지 않으므로, 애플리케이션이 실행 중에 행을 추가하는 경로는 없다.

자유 입력을 허용하면 같은 뜻의 태그가 표기만 달리해 흩어지고(`부산맛집`, `부산맛집추천`),
카테고리로 거르는 화면이 제 구실을 못 한다. 테이블 이름이 `hashtags` 인 것은 자유 입력을
받던 시절의 잔재다. `AdminStatsService` 가 네이티브 SQL 로 이 이름을 쓴다.
