...
## 2026-09-17 (문서의 해시태그 시절 설명 정정)

- API: 없음. 문서만 고쳤다
- 구분: 문서 정정
- 이전: `API_SPEC.md` 와 `ERD.md` 에 자유 입력 해시태그 시절 설명이 남아 있었다. "본문에 해시태그를 함께 적는다", "삭제할 때 연결을 지우고 복구할 때 본문에서 다시 뽑는다", "`#` 뒤의 한글·영문만 인정하고 게시물당 20개" 는 모두 지금 동작과 다르다.
- 이후: 카테고리는 `categories` 로 고른 값이고 본문과 무관하며, 소프트 삭제는 연결을 남기고 사용 수만 줄이고, 게시물당 최대 10개라는 현재 동작에 맞췄다.
- 이유: 두 문서는 HTTP 계약과 데이터 설계의 단일 기준인데, 틀린 설명을 보고 프론트가 본문에 `#` 을 적어 보낼 수 있다.
- 호환성 파괴: 없음. 코드는 바뀌지 않았다
- DB/ERD: `ERD.md` 24절(`hashtags`) 설명 갱신. 스키마 변경 없음

## 2026-09-17 (게시물 수정으로 빠진 사진 파일 정리)

- API: `PATCH /api/v1/posts/{postId}`
- 구분: 동작 변경. 요청·응답 형식은 변경 없음
- 이전: `mediaList` 를 교체하면 DB 행만 지우고 저장소의 파일은 남겨 뒀다. 고아 파일 정리는 한 번도 게시물에 붙지 않은 파일만 보므로, 붙었다 빠진 파일은 아무도 지우지 않았다.
- 이후: 교체로 더는 쓰이지 않는 파일을 저장소에서도 지운다. 삭제는 수정이 커밋된 뒤에 하고, 지우지 못해도 수정은 그대로 성공한다. 같은 `url` 을 다시 보낸 사진은 남긴다.
- 이유: 쓰지 않는 파일이 계속 쌓여 저장소 비용이 늘었다. 만료 게시물 정리가 이미 같은 방식으로 파일을 지운다.
- 호환성 파괴: 없음
- DB/ERD: 변경 없음

## 2026-09-16 (프로필 사진 URL 길이 제한, 지운 게시물 목록 size 검증)

- API: `PATCH /api/v1/users/me/profile-image`, `GET /api/v1/posts/me/deleted`
- 구분: 검증 강화
- 이전: 프로필 사진 URL 에 길이 제한이 없어 아주 긴 값(예: `data:` URI)도 저장됐다. 이 값은 피드·댓글·팔로워·알림 응답의 `author.profileImageUrl` 에 모두 실려 나간다. 지운 게시물 목록은 `size` 상한을 검증하지 않아 큰 값을 보내면 조용히 50 으로 줄여 응답했다.
- 이후: 프로필 사진 URL 은 최대 2048자(게시물 사진 URL 과 같은 값)이며 넘기면 `400` 이다. 지운 게시물 목록도 다른 목록과 같이 `size` 가 1~50 을 벗어나면 `400` 이다.
- 이유: 목록 응답마다 따라다니는 필드라 상한이 없으면 응답 크기를 한 사용자가 좌우한다. `size` 는 문서에 이미 "1 이상 50 이하, 넘기면 400" 이라고 적혀 있었고 이 엔드포인트만 지키지 않았다.
- 호환성 파괴: 없음. 정상 범위의 요청은 그대로 동작한다.
- DB/ERD: 변경 없음
...
- 구분: 추가
- 이전: 장소 상세는 DB 에 적재한 소개·운영정보만 보여줬다. TourAPI 장소는 카카오 장소 ID 가 없어 카카오맵으로 이을 수 없었다.
- 이후: 이름과 좌표로 같은 카카오 장소를 찾아 상세 페이지 주소를 준다. 못 찾으면 이름 검색 결과 페이지(`matched=false`)다.
- 이유: 장소 상세 화면을 후기·사진·영업 정보가 갱신되는 카카오맵으로 옮긴다. DB 상세는 "기본 정보" 탭으로 남긴다.
- 호환성 파괴: 없음

## 2026-09-15 (프로필에 차단 여부, 차단 응답에 팔로우 상태 추가)

- API: `GET /api/v1/users/{userId}/profile`, `POST /api/v1/users/{userId}/blocks`, `DELETE /api/v1/users/{userId}/blocks`
- 구분: 추가 (응답 필드)
- 이전: 프로필이 차단 여부를 주지 않아, 차단을 푼 뒤 프로필을 다시 열면 화면이 차단 버튼을 그리지 못했다. 차단 응답도 `blocked` 만 줘서 함께 끊긴 팔로우 상태를 알 수 없었다.
- 이후: 프로필에 `blocked`, 차단·해제 응답에 `following` 을 담는다.
- 이유: `following` 과 같은 방식이다. 화면이 프로필을 다시 부르지 않고 두 버튼을 함께 되돌릴 수 있다.
- 호환성 파괴: 없음. 필드를 더한다.
- DB/ERD: 변경 없음

## 2026-09-15 (인기 장소에 위시리스트 담김 여부 추가)

- API: `GET /api/v1/places/popular`
- 구분: 추가 (응답 필드)
- 이전: 담아 둔 장소인지 알 수 없어, 화면이 인기 장소와 내 위시리스트를 각각 불러 직접 대조해야 했다.
- 이후: `wishlisted` 를 함께 보낸다. 로그인하지 않았으면 `null` 이다. 장소 상세(`GET /places/{placeId}`)와 같은 규칙이다.
- 이유: `false` 를 주면 "안 담았다"와 "모른다"가 구분되지 않는다. 위시리스트가 길어져도 서버는 목록에 담긴 장소만 한 번에 확인한다.
- 호환성 파괴: 없음. 필드를 더한다.
- DB/ERD: 변경 없음

## 2026-09-15 (관리자 사용자 목록 역할 필터)

- API: `GET /api/v1/admin/users`
- 구분: 추가(파라미터)
- 이전: 역할로 거를 수 없어 관리자 목록을 보려면 전체 사용자를 받아 화면에서 걸러야 했다.
- 이후: `role`(`USER`, `ADMIN`)로 거른다. 생략하면 지금처럼 전부다.
- 이유: 관리자 콘솔에서 권한 변경을 사용자 제재와 다른 화면(운영자)으로 나눴다.
- 호환성 파괴: 없음
- DB/ERD: 변경 없음
...
- 구분: 변경 (요청 검증)
- 이전: `following` 이 아닌 값은 조용히 무시하고 전체 피드를 반환했다. 오타를 내도 목록이 나와 잘못 부른 줄 몰랐다.
- 이후: 쓸 수 있는 값은 `following` 뿐이며, 그 밖의 값은 `400 INVALID_POST_REQUEST` 다. 대소문자는 가리지 않고 빈 값은 보내지 않은 것과 같다.
- 이유: 인기 피드를 보려고 `?feed=popular` 를 보냈다가 최신 피드를 인기 피드로 오해한 일이 있었다. 인기 피드는 경로가 따로 있다.
- 호환성 파괴: 있음. 다만 프론트 저장소를 확인한 결과 `feed` 를 보내는 호출부가 없어 영향 범위가 없다.
- DB/ERD: 변경 없음
...
- 구분: 추가 (응답 필드)
- 이전: 댓글을 달아도 응답에 게시물 댓글 수가 없어 화면이 게시물을 다시 조회해야 숫자가 갱신됐다. 저장한 사람 수는 어디에도 없었다.
- 이후: 댓글 작성 응답에 `postCommentCount`, 게시물 상세에 `bookmarkCount` 를 담는다.
- 이유: 댓글을 달고 새로고침해야 수가 올라간다는 지적이 있었다. 저장 수는 작성자가 반응을 가늠할 수 있게 요청받았다.
- 호환성 파괴: 아니오. 필드가 느는 것이라 쓰지 않으면 이전과 같다.
- DB/ERD: 없음
- 관련 PR 또는 이슈: 없음

### 저장 수는 작성자에게만 보여준다

저장은 조용히 담아 두는 행동이다. 남의 글에서 몇 명이 담았는지 보이면 담는 것 자체가 눈치 보이는 일이 된다. 그래서 요청자가 작성자일 때만 숫자를 담고 남이 보면 `null` 이다.

- **목록 응답에는 넣지 않았다.** 여러 사람 글이 섞인 화면에서 내 글에만 숫자가 뜨면 들쭉날쭉해 보인다. 내 글만 모아 보는 목록도 아직 없다.
- **컬럼을 새로 두지 않았다.** 좋아요·댓글은 모두에게 보이고 피드마다 나가서 세어 둔 값을 쓰지만, 저장 수는 상세 화면에서 작성자 한 명에게만 나간다. 그때 세면 되고, 컬럼을 두면 저장·해제마다 갱신해야 하며 어긋났을 때 되돌릴 근거가 없다.
