...
일차 방문 수는 단일 고정값이 아니라 `absoluteMinimum`, `preferredMinimum`, `targetCount`, `maximum` 정책으로 계산한다. 6시간 이상일 때 여유 일정은 `3~4곳`, 보통 일정은 `4~5곳`, 빽빽한 일정은 최대 `5곳`을 탐색한다. 2시간 이하는 최소 체류시간과 이동 여유를 보장하기 위해 1곳, 그 이상은 기본 2곳을 절대 하한으로 두되, 후보가 부족하면 일차별 최소 1곳까지 낮출 수 있다. 여행 속도별 활동 시간 목표에서 60분의 예측 불가능 버퍼를 뺀 뒤 남는 큰 공백은 `feasibilityRisk`로 감점한다. 따라서 가용 시간이 긴 마지막 날은 가까운 추가 후보를 우선 검토하고, 후보가 없거나 실제 경로가 길면 더 적은 장소 수를 정상 결과로 허용한다.

실제 상세 경로를 적용한 뒤 시간 초과가 확인되면 선택 장소를 바로 삭제하지 않는다. `RouteReorderRepair → StayDurationRepair → NearbyReplacementRepair → CrossDayMoveRepair → LowUtilityRemovalRepair` 순으로 소수의 후보를 만들고, 각 후보는 전체 일차를 다시 경로·시간 feasibility로 검증한다. 마지막 제거는 필수 장소와 고정 행사를 보존하며, 실제 대중교통과 식사 대기 때문에 정책 최소 장소 수도 불가능한 경우에만 1곳까지 허용한다.
...
- 기본 속도는 8시간 이상일 때 4곳, 6시간 이상일 때 4곳을 목표로 한다. `PACE_RELAXED`는 가용시간 6시간 이상일 때 3곳, 4시간 이상일 때 3곳을 목표로 한다. `PACE_PACKED`는 8시간 이상일 때 5곳, 6시간 이상일 때 4곳을 목표로 한다. 2시간 이하는 1곳을 목표로 한다. 목표보다 적은 일정은 장시간 행사·필수 장소·실제 이동시간으로 feasibility가 우선될 때 허용한다.
